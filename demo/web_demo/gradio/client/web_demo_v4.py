...
def chat(img_b64, msgs, ctx, params=None, vision_hidden_states=None):
    default_params = {"num_beams": 3,
                      "repetition_penalty": 1.2, "max_new_tokens": 1024}
    if params is None:
...
def encode_video(video):
    ...
    video = [encode_image(v)[0] for v in video]
    return video
...
def respond(_question, _chat_bot, _app_cfg, params_form):
    ...
    _context.append({'role': 'user', 'contents': encode_message(_question)})
    ...
    if files_cnts[1] + videos_cnt > 0:
        ...
        params["max_slice_nums"] = 1 if count_video_frames(
            _context) > 16 else 2
...
def regenerate_button_clicked(_question, _image, _user_message, _assistant_message, _chat_bot, _app_cfg, params_form):
    ...
    if _app_cfg["chat_type"] == "Chat":
        ...
        upload_image_disabled = videos_cnt > 0
        upload_video_disabled = videos_cnt > 0 or images_cnt > 0
        _question, _chat_bot, _app_cfg = respond(
            _question, _chat_bot, _app_cfg, params_form)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3, min_width=500):
                app_session = gr.State(
                    {'sts': None, 'ctx': [], 'images_cnt': 0, 'videos_cnt': 0, 'chat_type': 'Chat'})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Web Demo for MiniCPM-V 4.0')
